...
import { BankService } from '../classes/services/bank.service'
...
export class BankFormComponent {
    ...
    constructor(private activatedRoute: ActivatedRoute, private bankService: BankService, private dexieService: DexieService, private dialogService: DialogService, private formBuilder: FormBuilder, private helperService: HelperService, private messageDialogService: MessageDialogService, private messageHintService: MessageInputHintService, private messageLabelService: MessageLabelService, private router: Router) { }
    ...
    public onDelete(): void {
        this.dialogService.open(this.messageDialogService.confirmDelete(), 'question', ['abort', 'ok']).subscribe(response => {
            if (response) {
                this.bankService.delete(this.form.value.id).subscribe({
                    complete: () => {
    ...
    private saveRecord(bank: BankWriteDto): void {
        this.bankService.save(bank).subscribe({
            next: (response) => {
                this.dexieService.update('banks', { 'id': response.id, 'description': bank.description, 'isActive': bank.isActive })
                this.helperService.doPostSaveFormTasks(this.messageDialogService.success(), 'ok', this.parentUrl, true)
            },
            error: (errorFromInterceptor) => {
                this.dialogService.open(this.messageDialogService.filterResponse(errorFromInterceptor), 'error', ['ok'])
