...
namespace API.Features.RetailSales {
    ...
    public class RetailSaleXmlRepository : IRetailSaleXmlRepository {

        public string CreateXMLFileAsync(XmlRetailSaleVM invoice) {
            var fullpathname = FileSystemHelpers.CreateRetailSaleFullPathName(invoice.InvoiceHeader, "Xmls", "invoice");
        ...
        public string SaveInvoiceResponse(XmlRetailSaleHeaderVM invoiceHeader, string subdirectory, string response) {
            using StreamWriter outputFile = new(FileSystemHelpers.CreateRetailSaleFullPathName(invoiceHeader, subdirectory, "response"));
